...
public class ModifyPackage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(rs.next()){
				session.setAttribute("packageName1", rs.getString(2));
				session.setAttribute("placeName1", rs.getString(3));
				session.setAttribute("cost1", rs.getString(4));
				session.setAttribute("days1", rs.getString(5));
				session.setAttribute("persons1", rs.getString(6));
				session.setAttribute("totalCost1", rs.getInt(7));
