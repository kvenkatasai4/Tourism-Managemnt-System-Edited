	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(rs.next()){
				...
				session.setAttribute("noOfRooms1", rs.getInt(5));
				session.setAttribute("totalCost1", rs.getInt(6));
				session.setAttribute("roomDate1", rs.getString(7));
				session.setAttribute("packagename", rs.getString(9));
				session.setAttribute("place", rs.getString(10));
